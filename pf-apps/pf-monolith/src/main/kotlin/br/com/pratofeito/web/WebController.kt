...
class WebController(
    ...
    // RESTAURANTS
    @MessageMapping("/restaurants/createcommand")
    fun createRestaurant(request: CreateRestaurantDTO) {
        ...
        commandGateway.send(
            CreateRestaurantCommand(request.name, menu, auditEntry),
            LoggingCallback.INSTANCE
        )
    }
    ...
    fun markCourierOrderAsDelivered(id: String) = commandGateway.send(
        MarkCourierOrderAsDeliveredCommand(CourierOrderId(id), auditEntry),
        LoggingCallback.INSTANCE
    )
...
data class CreateOrderDTO(val customerId: String, val restaurantId: String, val orderItems: List<OrderItemsDTO>)

data class OrderItemsDTO(val id: String, val name: String, val price: BigDecimal, val quantity: Int)
