...
internal class Restaurant {
    ...
    fun handle(command: CreateRestaurantOrderCommand) {
        if (menu.menuItems.stream().map { mi -> mi.id }.collect(Collectors.toList())
                .containsAll(command.orderDetails.lineItems.stream().map { li -> li.menuItemId }
                    .collect(Collectors.toList()))
        ) {
            AggregateLifecycle.createNew(RestaurantOrder::class.java) {
                RestaurantOrder(command)
            }
        } else {
