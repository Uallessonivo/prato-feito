...
class RestaurantAggregateTest {
    ...
    fun createRestaurantOrderTest() {
        ...
        val restaurantCreatedEvent =
            RestaurantCreatedEvent(name, menu, restaurantId, auditEntry)

        val createRestaurantOrderCommand =
            CreateRestaurantOrderCommand(restaurantId, orderDetails, orderId, auditEntry)
        val restaurantOrderCreatedEvent = RestaurantOrderCreatedEvent(lineItems, orderId, restaurantId, auditEntry)

        fixture.given(restaurantCreatedEvent).`when`(createRestaurantOrderCommand)
